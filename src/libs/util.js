...
const { title, cookieExpires, useI18n, cookieToken } = config

export const TOKEN_KEY = cookieToken
...
export const showNames = (item, vm) => {
  let { title, __titleIsFunction__ } = item.meta
  if (!title) return
  if (useI18n) {
    if (title.includes('{{') && title.includes('}}') && useI18n) {
    ...
    else title = vm.$t(item.calos)
  } else title = (item.meta && item.meta.title) || item.calos
  return title
}
